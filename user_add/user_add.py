...
def delete_na_rows(syn, input_table):
    rows_to_delete = syn.tableQuery(
        "select * from {} where phone_number is null or guid is null".format(
            input_table
        )
    )
...
def get_new_users(syn, input_table, output_table):
    input_table_df = syn.tableQuery(
        "select * from {}".format(input_table)
    ).asDataFrame()
    for i, user in input_table_df.iterrows():
        if pd.isnull(user.phone_number) and pd.isnull(user.guid):
            ...
            return (
                "Error: phone_number and guid was left blank",
                -1,
                -1,
                user.visit_date,
            )
            ...
            delete_na_rows(syn, input_table)
            return (
                "Error: phone_number was left blank",
                -1,
                user.guid,
                user.visit_date,
            )
    ...
    output_table_df = syn.tableQuery(
        "select phone_number, guid from {}".format(output_table)
    ).asDataFrame()
...
def get_participant_info(bridge, phone_number):
    participant_info = bridge.restPOST(
        "/v3/participants/search", {"phoneFilter": phone_number}
    )
    return participant_info


def process_request(
    bridge, participant_info, phone_number, external_id, substudy, support_email
):
    if participant_info["total"] == 0:
        ...
        try:
            ...
            bridge.restPOST(
                "/v3/participants",
                {
                    ...
                    "phone": {"number": phone_number, "regionCode": "US"},
                    "dataGroups": engagement_groups
                    + ["clinical_consent", "show_3_cognitive"],
        ...
        if not (
            "gr_SC_DB" in user_info["dataGroups"]
            or "gr_SC_CS" in user_info["dataGroups"]
        ):
            user_info["dataGroups"] = user_info["dataGroups"] + [
                random.choice(["gr_SC_DB", "gr_SC_CS"])
            ]
        if not (
            "gr_BR_AD" in user_info["dataGroups"]
            or "gr_BR_II" in user_info["dataGroups"]
        ):
            user_info["dataGroups"] = user_info["dataGroups"] + [
                random.choice(["gr_BR_AD", "gr_BR_II"])
            ]
        if not (
            "gr_ST_T" in user_info["dataGroups"] or "gr_ST_F" in user_info["dataGroups"]
        ):
            user_info["dataGroups"] = user_info["dataGroups"] + [
                random.choice(["gr_ST_T", "gr_ST_F"])
            ]
        if not (
            "gr_DT_F" in user_info["dataGroups"] or "gr_DT_T" in user_info["dataGroups"]
        ):
            user_info["dataGroups"] = user_info["dataGroups"] + [
        ...
        try:
            ...
            return (
                "Error: Preexising user account found. "
        ...
        if not "at-home-pd" in participant_info["items"][0]["externalIds"]:
            try:
                bridge.restPOST(
                    "/v5/studies/{}/enrollments".format(substudy),
                    {
                        "userId": user_id,
            ...
            return "Success: Preexisting user account found. " "Enrolled in new study."
        return (
            "Success: Preexisting user account found. "
        ...
        return (
            "Error: Preexisting account found with GUID {}. "
            "Please contact {} "
        ...
        return (
            "Success: Preexisting account found with matching phone number " "and guid."
        )


def create_table_row(status, phone_number, guid, visit_date, output_table):
...
def is_valid_guid(guid, substudy):
    ...
    if p is not None:
        ...
        print(f"substudy {substudy} is invalid.")
...
def main():
    ...
    new_users = get_new_users(
        syn,
        input_table=credentials["inputTable"],
    ...
    if any(duplicated_numbers):
        ...
        if len(duplicates.guid) == len(duplicates.guid.unique()):
            table_row = create_table_row(
                "Error: It looks like you accidentally "
    ...
    for i, user in new_users.iterrows():
        ...
        try:
            if not is_valid_phone_number(phone_number):
                table_row = create_table_row(
                    "Error: The phone number is improperly "
                ...
                bridge = get_bridge_client(
                    credentials["bridgeUsername"], credentials["bridgePassword"]
                )
                ...
                status = process_request(
                    ...
                    phone_number,
                    guid,
                    credentials["substudy"],
                    credentials["supportEmail"],
                )
                table_row = create_table_row(
                    status, phone_number, guid, visit_date, credentials["outputTable"]
                )
        except Exception as e:
            table_row = create_table_row(
                "Error: One of the fields is improperly "
